    ...
    >
      <template slot="items" slot-scope="props">
        ...
        <td class="justify-center layout px-0">
          ...
          <v-btn icon @click="deleteGood(props.item)">
            <i class="el-icon-delete"/>
          </v-btn>
          <v-btn icon v-if="props.item.saleable" @click="changeSaleable(props.item)">下架</v-btn>
          <v-btn icon v-else @click="changeSaleable(props.item)">上架</v-btn>
        </td>
      </template>
    </v-data-table>
    <!--弹出的对话框-->
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      deleteGood(good){
        let _this = this;
        this.$http.delete("/item/goods/del/"+good.id).then(() => {
            console.log(_this);
      ...
      changeSaleable(good){
        let data = {spuId:good.id,status:!good.saleable};
        this.$http.put("/item/goods/status",this.$qs.stringify(data)).then(
          () =>{
            this.filter.saleable = !good.saleable;
          }
        )
      },
