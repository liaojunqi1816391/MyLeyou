...
public class SearchService {
    ...
    public Goods buildGoods(Spu spu) throws IOException {
        ...
        params.forEach(param -> {
            ...
            if (param.getGeneric()) {
                // 获取通用规格参数值
                String value = ObjectUtils.toString(genericSpecMap.get(param.getId()));
                // 判断是否是数值类型
                if (param.getNumeric()){
                    // 如果是数值的话，判断该数值落在那个区间
...
//        MatchQueryBuilder basicQuery = QueryBuilders.matchQuery("all", request.getKey()).operator(Operator.AND);
        BoolQueryBuilder booleanQueryBuilder = buildBooleanQueryBuilder(request);
        ...
        if (categories.size() == 1) {
            specs = getParamAggResult((Long)categories.get(0).get("id"), booleanQueryBuilder);
        }
